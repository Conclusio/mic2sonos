            ...
            routing {
                ...
                get("/mic.wav") {
                    ...
                    try {
                        rawPcmFlow.collect { chunk ->
                            ...
                            for (i in 0 until chunk.size / 2) {
                                val sample =
                                    (chunk[i * 2].toInt() and 0xFF) or (chunk[i * 2 + 1].toInt() shl 8)
                    ...
                    for (i in 0 until rawPcm.size / 2) {
                        val sample =
                            (rawPcm[i * 2].toInt() and 0xFF) or (rawPcm[i * 2 + 1].toInt() shl 8)
                        val signed = sample.toShort().toInt()
                        ...
                        try {
                            ...
                            rawPcmFlow.collect { pcmChunk ->
                                ...
                                for (i in 0 until pcmChunk.size / 2) {
                                    val sample =
                                        (pcmChunk[i * 2].toInt() and 0xFF) or (pcmChunk[i * 2 + 1].toInt() shl 8)
                                    val signed = sample.toShort().toInt()
                                ...
                                if (chunkCount % 100 == 0) {
                                    Log.d(
                                        TAG,
                                        "Streamed $chunkCount chunks, ${totalBytes / 1024}KB, gain=$gain"
                                    )
                                }
                            }
                        } catch (e: IOException) {
                ...
                get("/test-static.wav") {
                    ...
                    Log.i(
                        TAG,
                        "Generated static WAV: ${wavBytes.size} bytes, $durationSeconds seconds"
                    )
...
"""
                ...
                get("/stream.aac") {
                    ...
                    Log.i(
                        TAG,
                        "Request headers: ${
                            call.request.headers.entries().map { "${it.key}: ${it.value}" }
                        }"
                    )
                        ...
                        try {
                            ...
                            audioDataFlow.collect { audioChunk ->
                                ...
                                if (chunkCount % 50 == 0) {
                                    Log.d(
                                        TAG,
                                        "Streamed $chunkCount chunks, $totalBytes bytes total"
                                    )
                                }
                            }
                        } catch (_: IOException) {
            ...
            while (isActive && isRunning.get()) {
                try {
                    ...
                    for (i in 0 until bytesRead / 2) {
                        val sample =
                            (pcmBuffer[i * 2].toInt() and 0xFF) or (pcmBuffer[i * 2 + 1].toInt() shl 8)
                    ...
                    if (maxSample > 100) {
                        ...
                        if (System.currentTimeMillis() % 2000 < 50) {
                            Log.d(
                                TAG,
                                "PCM data max sample: $maxSample (should be > 0 if mic is capturing)"
                            )
                        }
                    }
    ...
    private fun addPcmToHlsSegment(pcmChunk: ByteArray) {
        synchronized(hlsLock) {
            ...
            if (elapsed >= HLS_SEGMENT_DURATION_MS) {
                ...
                Log.d(
                    TAG,
                    "Created HLS segment ${segment.id}: ${segmentData.size} bytes, ${elapsed}ms"
                )
